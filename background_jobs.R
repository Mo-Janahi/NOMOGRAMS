...
durations_table_2 <- data.frame( traning_size=double() ,  duration_sw=double() ,  diff_sw=double() , duration_gpr=double() , diff_gpr=double() )
bins_male_sw <- WINDOW_ANALYSIS( ukb_img_ex_outliers_male ) 

for(num_train_samples in seq(200,3600,100) ){
  ...
  assign ( paste("bins_male_gpr_",num_train_samples,sep="") ,
           read.table(paste("~/Desktop/UKB NOMOGRAM PROJECT/Scripts/NOMOGRAMS/TEMP_GPR_MODELS/bins_male_gpr_compare_",num_train_samples,sep="") ) )
  ...
  durations_table_2[nrow(durations_table_2) + 1 , ] <- c( num_train_samples , duration_sw , dif_sw , duration_gpr  , dif_gpr )
  ...
  #write.table( durations_table , paste( "~/Desktop/UKB NOMOGRAM PROJECT/Scripts/NOMOGRAMS/TEMP_GPR_MODELS/","durations_compare",sep="" ) )
  
  png(file=paste("~/Desktop/UKB NOMOGRAM PROJECT/Scripts/NOMOGRAMS/TEMP_GPR_MODELS/all_compared_",num_train_samples,".png",sep = ""), width=1500, height=875)
  par(mfrow=c(2,2))
